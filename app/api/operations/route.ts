...
export async function POST(request: Request) {
  try {
    ...
    if (body.type === "order") {
      const [row] = await db.insert(orders).values({ customerName: String(body.customerName || "Pedido administrativo"), phone: String(body.phone || ""), items: JSON.stringify(body.items || []), total: Number(body.total || 0), discount: Number(body.discount || 0), deliveryFee: Number(body.deliveryFee || 0), status: "Confirmado", source: "admin", createdBy: access.email, clientId: body.clientId ? Number(body.clientId) : null, notes: String(body.notes || "") }).returning();
