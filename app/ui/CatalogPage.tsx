...
export default function CatalogPage(){
 const [products,setProducts]=useState<Product[]>(catalog),[filter,setFilter]=useState("Todos"),[query,setQuery]=useState(""),[cart,setCart]=useState<CartItem[]>([]),[open,setOpen]=useState(false),[customer,setCustomer]=useState("");
 ...
 return <main className="catalog-page">
  ...
  <aside className="fee-note"><b>Importante</b><span>Para trabalhos com preço inferior a R$ 50, cobramos taxa de R$ 20 para buscar e entregar.</span></aside>
  <footer className="catalog-footer"><Link href="/">Voltar para a bio</Link><span>Laboratório Orto Fernandes · 98535-6011</span></footer>
  {count>0&&<button className="floating-cart" onClick={()=>setOpen(true)}><span className="cart-bag">▣</span><span><b>Ver carrinho</b><small>{count} {count===1?"item":"itens"}</small></span><strong>{money(total)}</strong></button>}
  {open&&<div className="cart-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}><aside className="cart-drawer"><header><div><span className="kicker">SUA ENCOMENDA</span><h2>Revisar carrinho</h2></div><button onClick={()=>setOpen(false)} aria-label="Fechar carrinho">×</button></header><div className="cart-list">{cart.map(item=><article className="cart-row" key={item.id}><div><b>{item.name}</b><small>{money(item.price)} cada</small></div><div className="quantity"><button onClick={()=>change(item.id,-1)} aria-label={`Diminuir ${item.name}`}>−</button><span>{item.quantity}</span><button onClick={()=>change(item.id,1)} aria-label={`Aumentar ${item.name}`}>+</button></div><strong>{money(item.price*item.quantity)}</strong></article>)}</div><div className="cart-summary"><div><span>Itens</span><b>{count}</b></div><div><span>Valor estimado</span><b>{money(total)}</b></div></div><label className="customer-name">Nome ou clínica <input value={customer} onChange={e=>setCustomer(e.target.value)} placeholder="Opcional"/></label><a className="finish-whatsapp" href={wa(orderMessage())} target="_blank"><span>◉</span><span><b>Enviar pedido pelo WhatsApp</b><small>Uma mensagem com todos os itens será criada</small></span></a><button className="clear-cart" onClick={()=>{setCart([]);setOpen(false)}}>Limpar carrinho</button></aside></div>}
 </main>
}
